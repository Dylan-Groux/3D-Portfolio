...
const ProjectDetails = ({ title, description, subDescription, image, tags, href, closeModal }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center items-center w-full h-full overflow-hidden backdrop-blur-sm">
        <motion.div className="relative max-w-2xl border shadow-sm rounded-2xl bg-linear-to-l from-midnight to-navy border-white/10" 
        ...
        animate={{ opacity: 1, scale: 1 }}>
            ...
            <div>
                ...
                <div className="flex items-center justify-between mt-4">
                    <div>
                        {tags.map((tag)=>
                         <img key={tag.id} src={tag.logo} alt={tag.name} className="w-9 h-5 mb-1 ml-3 inline-block hover-animation" />
                        )}
                    </div>
                    <a href={href} className="inline-flex items-center gap-1 mr-2 font-medium hover-animation cursor-pointer">Voir le projet<img src="assets/arrow-up.svg" alt="arrow-up" className="size-4" /></a>
                </div>
            </div>
        </motion.div>
    </div>
